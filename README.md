...
### A job that is not finished says where it is

`jc_jobs_get`, `jc_jobs_submit_and_wait` and `jc_scrape_result` all answer a
not-yet-finished job with the same `result` shape, read off the job's own
execution trace rather than from a clock:
