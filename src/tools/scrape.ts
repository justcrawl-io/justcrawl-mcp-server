...
import { describeJobResult, describePending, describeStillRunning, refetchForTrace, resolveUrlItem } from './jobs.js';
      ...
      run('jc_scrape_result', async () => {
        ...
        try {
          ...
          if (err instanceof JobWaitTimeoutError) {
            // One re-fetch so the note can name the vendor or the queue wait —
            // this is the tool a user is actually watching, so "where is it"
            // matters more here than anywhere else.
            return jsonResult({
              ...describeStillRunning(await refetchForTrace(client, err), err.elapsedMs),
              nextStep:
        ...
        if (job.status !== COMPLETED) {
          return jsonResult({ jobId, job, result: describePending(job) });
        }
  ...
  server.registerTool(
    ...
    async ({ jobId, attributes, pageType }) =>
      run('jc_scrape_extract', async () => {
        ...
        const job = (await client.jobs.get(jobId)) as { url?: string; workflowId?: string; status?: string };
        if (typeof job.url !== 'string') {
          return toolError('That job carries no URL, so there is no domain to attach a schema to.');
        }
        // Checked BEFORE the workflow rebuild below, which regenerates this
        // domain's DAG from the template: reaching it with a job that has no
        // page yet discards a hand-edited workflow and only then fails on the
        // gateway's not-ready 409 — the org changed, the caller with nothing.
        // `jc_scrape_result` already gates on this status; this is the sibling
        // that did not, which is how a slow job came back as "page gone".
        if (job.status !== undefined && job.status !== COMPLETED) {
          return toolError(
            `That job is ${job.status}, so it has no page to extract from yet. Call jc_scrape_result with ` +
              `jobId="${jobId}" until it returns content, then call this tool again.`,
          );
        }
